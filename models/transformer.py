...
class PyramidalPositionalEncoding(nn.Module):
    def __init__(self, patch_sizes, num_channels, dim):
        ...
        self.cnns = nn.ModuleList([
            nn.Conv2d(num_channels, dim, kernel_size=patch_size, padding=patch_size // 2 - 1 if patch_size % 2 == 0 else patch_size // 2)
...
class Transformer(nn.Module):
    ...
    def forward(self, style, mask , content, pos_embed_c, pos_embed_s):
        ...
        print("content Cape",content.shape)
        pos_embed_c = self.ppe(content)
        '''
        if models.StyTR.content_style:
            for i in range(512):
                pyplot.imshow(pos_embed_c[0, i, :, :], cmap='viridis')
                pyplot.savefig(f"D:/finalyearproject/project/intermediate_outputs/cape_outputs/" + str(i) + ".png")
        '''
        ...
        if pos_embed_c is not None:
            ...
            print("posembedc",pos_embed_c.shape)
        ...
        print("Content_encoder", content.shape)
        print("Style_encoder", style.shape)
        print("hs_shape", hs.shape)
